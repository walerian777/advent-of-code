...
func part2() int {
	...
	areas := make(map[*Region]int)
	sides := make(map[*Region]int)

	for k, v := range gardens {
		areas[v]++
		edges := make(map[Garden]byte)
		for dir, n := range neighborsDir(&k, maxX, maxY) {
			if r := gardens[n]; r != v {
				edges[n] = dir
			}
		}
...
func neighborsDir(c *Garden, maxX, maxY int) map[byte]Garden {
	ns := make(map[byte]Garden)
	// north
	if c.x > 0 {
		ns['n'] = Garden{x: c.x - 1, y: c.y}
	}
	// east
	if c.y < maxY {
		ns['e'] = Garden{x: c.x, y: c.y + 1}
	}
	// south
	if c.x < maxX {
		ns['s'] = Garden{x: c.x + 1, y: c.y}
	}
	// west
	if c.y > 0 {
		ns['w'] = Garden{x: c.x, y: c.y - 1}
